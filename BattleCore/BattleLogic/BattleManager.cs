    ...
    public class BattleManager
    {
        ...
        public static bool BattleSimulation(Fighter source, Fighter taker)
        {
            while (true)
            {
                ...
                if (source.SpeedBar >= Max_SpeedBar)
                {
                    ...
                    JsonLogger.LogRoundBegin(source.Name);
                    //Thread.Sleep(1000);
                ...
                if (taker.SpeedBar >= Max_SpeedBar)
                {
                    ...
                    JsonLogger.LogRoundBegin(taker.Name);
                    //Thread.Sleep(1000);
        ...
        public static List<AwardInfo> SetBattleResult(User challenger, User opponent, bool isWin,bool isHome = true)
        {
            ...
            if (challenger.Id == opponent.Id)
            {
                ...
                JsonLogger.LogBattleEnd(opponent, challenger, isWin ? challenger.Name : opponent.Name);
            ...
            else
            {
                ...
                Func<User, User, Boolean, Boolean, AwardInfo> SettleResult = (challenger, opponent, isWin, isHome) =>
                {
                    ...
                    challenger.Exp = exp;
                    if (isHome)
                        JsonLogger.LogBattleEnd(challenger_copy!, challenger, isWin ? challenger.Name : opponent.Name);
                    return awardInfo;
                };
                return new List<AwardInfo>
                {
